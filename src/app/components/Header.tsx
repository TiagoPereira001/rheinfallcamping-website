    ...
    <header className="bg-white border-b border-black/8 sticky top-0 z-50 pt-[env(safe-area-inset-top)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
      <div className="max-w-7xl mx-auto px-5 lg:px-10 h-16 flex items-center justify-between gap-4">
        <NavLink
          ...
          className="font-serif text-[1.1rem] sm:text-[1.25rem] font-semibold text-black tracking-tight select-none hover:opacity-70 transition-opacity truncate"
        >
          RheinfallCamping
        </NavLink>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `text-[0.8125rem] font-normal tracking-wide transition-opacity duration-200 ${
